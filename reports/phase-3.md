...
Because there is an assumption that clients are not trustworthy, all clients (C) must be verified via a password before being issued a token.  When an administrator (A) first creates a user, the server (S) issues a one-time password of 8 randomized characters. A derived key of the password is then stored on the server, created with PBKDF2 and salt consisting of 8 random characters, which will also be stored on the server. This password is communicated to the new user via the administrator in person, and upon their first successful entry to the server they are told to enter their new password.  They will then use this password for all subsequent attempts to enter the server. At this point a new derived key is created consisting of the new password and previous salt, still using PBKDF2.  This will be stored on the server and will serve as the basis for verifying a user’s password.
...
The suggested protocol gives a secure level of protection against unauthorized clients attempting to access the file system illegitimately.  If an adversary attempts to perform a dictionary attack against the server, any weakness in the user’s passwords will be countered by said passwords being both hashed and salted, which will lead to the derived keys being stored locally being unrecognizable from any of the common words or phrases that might be tried.  The salted nature of the derived key also protects against a rainbow table attack, as the salt will force the hash dictionary to recompute for every password.  The use of PBKDF2 also provides benefits against a brute force attack. This is because PBKDF2 performs a SHA-256 encryption over multiple iterations of the password, which stretches the key to such a large size that it is infeasible to be broken by brute force within a reasonable amount of time.
